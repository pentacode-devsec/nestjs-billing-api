...
export interface CreditNote {
  ...
  issueTime: string;
  currency: string;
  discrepancyCode: string; // motivo de la NC
  discrepancyReason: string; // descripción del motivo
  billingReference: string; // número de la factura que corrige
  billingReferenceCufe: string; // CUFE de la factura que corrige
...
export class CreditNoteGenerator {
  generate(note: CreditNote): string {
    ...
    note.items.forEach((item) => {
      this.addCreditNoteLine(doc, item, note.currency);
    });
  ...
  private addCreditNoteLine(
    ...
    line
      .ele('cbc:CreditedQuantity', { unitCode: 'EA' })
